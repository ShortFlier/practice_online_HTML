...
import {paper_looks} from "@/utils/api_path";
...
import {identity_admin, identity_teacher, reply_all} from "@/utils/constant";
...
onMounted(()=>{
  if(!(store.state.identity==identity_admin||store.state.identity==identity_teacher))
    router.push('/login')
  if(!isLogin())
    router.push('/login')
  const id=useRoute().params.id
  axios.get(paper_looks+id).then(resolve=>{
    if (resolve){
